...
map(
  { "n", "i", "v", "x" },
  "<F5>",
  '<ESC><cmd>TermExec cmd="cc % -o temp.out;./temp.out;rm ./temp.out"<CR>',
  { desc = "Run C" }
)
...
map("i", comment_key, function()
  require("Comment.api").toggle.linewise.current()
end)
map("n", comment_key, function()
  require("Comment.api").toggle.linewise.current()
end)
map("x", comment_key, function()
